...
useEffect(() => {
  ...
  return (
    <div className="h-screen bg-[var(--color-background)]  flex overflow-hidden"> 
      ...
      <div className="w-80 bg-[var(--color-surface)] dark:bg-gray-800 border-r border-[var(--color-surface)]  flex flex-col h-full ">
        {/* Header */}
        <div className="p-4 border-b border-[var(--color-surface)] flex-shrink-0">
          ...
          <h2 className="text-lg font-semibold text-[var(--color-text)]  mb-4">Chats</h2>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-gray-700 border border-[var(--color-surface)] dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600  text-[var(--color-text)] placeholder-gray-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.map((conversation) => (
            <div 
              ...
              className={`p-4 hover:bg-[var(--color-background)] dark:hover:bg-gray-700 cursor-pointer transition-colors border-b  border-[var(--color-surface)] last:border-b-0 ${
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-sm truncate ${
                    conversation.unread ? 'text-[var(--color-text)]  font-medium' : 'text-gray-600 dark:text-gray-400'
                  }`}>
                    {conversation.lastMessage}
                  </p>
                </div>
                {conversation.unread && (
                  <div className="bg-[var(--color-primary)] text-white text-xs px-2 py-1 rounded-full min-w-[20px] text-center">
                    {conversation.unread}
                  </div>
                )}
              </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {selectedConversation && selectedConv ? (
          <>
              ...
              <div className="absolute inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50">
                <div className="text-center text-white">
                  <div className="w-32 h-32 bg-gray-600 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <div className="w-24 h-24 rounded-full bg-[var(--color-primary)] flex items-center justify-center text-white text-2xl font-semibold">
            ...
            {/* Chat Header */}
            <div className="p-4 bg-[var(--color-surface)] dark:bg-gray-800 border-b  border-[var(--color-surface)] flex items-center justify-between flex-shrink-0 fixed w-full">
              <div 
                className="flex items-center space-x-3 cursor-pointer hover:bg-[var(--color-background)] dark:hover:bg-gray-700 rounded-lg p-2 -m-2 transition-colors"
              ...
              >
                ...
                <div>
                  <h3 className="font-medium text-[var(--color-text)] ">{selectedConv.user.name}</h3>
            ...
            <div className="flex flex-col flex-1 h-full pt-40">
              {/* Messages Area */}
                    ...
                    >
                      <div className={`max-w-sm px-4 py-2 rounded-2xl ${
                        message.senderId === 'me' 
                          ...
                          : 'bg-[var(--color-surface)] dark:bg-gray-700 text-[var(--color-text)]  border border-[var(--color-surface)] dark:border-gray-600'
                      }`}>
              ...
              {showSidebar && (
                <div className="w-80 bg-[var(--color-surface)] dark:bg-gray-800 border-l  border-[var(--color-surface)] p-6 overflow-y-auto">
                  <div className="text-center">
                    <div className="w-24 h-24 rounded-full bg-[var(--color-primary)] flex items-center justify-center text-white text-2xl font-semibold mx-auto mb-4">
                      {selectedConv.user.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    ...
                    <div className="flex justify-center space-x-6 mb-6">
                      <button 
                        ...
                        className="flex flex-col items-center space-y-2 text-gray-600 dark:text-gray-400 hover:text-[var(--color-primary)] transition-colors"
                      >
                      ...
                      <button className="flex flex-col items-center space-y-2 text-gray-600 dark:text-gray-400 hover:text-[var(--color-primary)] transition-colors">
                        <div className="w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center">
            ...
            <div className="p-4 bg-[var(--color-surface)] dark:bg-gray-800 border-t  border-[var(--color-surface)] flex-shrink-0 relative">
              <div className="flex items-center space-x-3">
                <button 
                ...
                <button className="text-gray-500 hover:text-[var(--color-primary)] transition-colors">
                ...
                <div className="flex-1 flex items-center bg-gray-100 dark:bg-gray-700 rounded-full px-4 py-3">
                  <input
                    ...
                    className="flex-1 bg-transparent outline-none text-[var(--color-text)]  placeholder-gray-500"
                  />
                  <button 
                    onClick={() => setShowEmojiPicker(!showEmojiPicker)}
                    className={`ml-2 transition-colors ${showEmojiPicker ? 'text-[var(--color-primary)]' : 'text-gray-500 hover:text-[var(--color-primary)]'}`}
                    title="Add emoji"
                  >
                ...
                <button 
                  ...
                  className="bg-[var(--color-primary)] hover:bg-[var(--color-accent)] disabled:bg-gray-300 disabled:cursor-not-allowed text-white p-3 rounded-full transition-colors"
                >
              ...
              {/* Emoji Picker */}
              {showEmojiPicker && (
                <div className="absolute bottom-20 right-4 w-64 bg-[var(--color-surface)] dark:bg-gray-800 border border-[var(--color-surface)] dark:border-gray-600 rounded-lg shadow-xl z-50">
                  <div className="p-3">
          ...
          <div className="flex-1 flex items-center justify-center bg-[var(--color-background)] ">
            <div className="text-center max-w-md">
              <div className="w-24 h-24 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center mx-auto mb-6">
                <Send className="w-12 h-12 text-[var(--color-primary)]" />
              </div>
              ...
              <button 
                ...
                className="bg-[var(--color-primary)] hover:bg-[var(--color-accent)] text-white px-6 py-3 rounded-lg font-medium transition-colors"
              >
                Send Message
              </button>
            </div>
